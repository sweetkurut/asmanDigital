...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setStatus("error");
        setErrorMessage(
          data.errors?.join(", ") || data.error || t("contact.error"),
        );
        setTimeout(() => setStatus("idle"), 5000);
      }
    } catch {
  ...
  const inputBaseClass =
    "w-full px-4 py-3 bg-dark border rounded-lg focus:outline-none focus:ring-2 text-light transition-all";
  ...
  const getInputClass = (field: keyof FormData) =>
    `${inputBaseClass} ${errors[field] && touched[field] ? inputErrorClass : inputNormalClass}`;
              ...
              <div className="card p-8 space-y-8">
                ...
                <div className="pt-6 mt-6 border-t border-gray-800">
                  ...
                  <div className="flex space-x-4">
                    <a
                      href="https://www.instagram.com/asmandigital/"
                      target="_blank"
